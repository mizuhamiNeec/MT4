...
Mat4 Mat4::RotateAxisAngle(const Vec3& axis, const float angle) {
	Mat4 result = identity;
	...
	result.m[0][1] = y * x * oneMinusCos + z * sin;
	result.m[0][2] = z * x * oneMinusCos - y * sin;

	result.m[1][0] = x * y * oneMinusCos - z * sin;
	result.m[1][1] = cos + y * y * oneMinusCos;
	result.m[1][2] = z * y * oneMinusCos + x * sin;
